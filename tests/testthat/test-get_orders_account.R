...
test_that("invalid class type for tokens throws error", {
  expect_error(get_orders_account(1,
               ...
               "Tokens must be a list, encrypted account ID, from/to entered times, and status must be strings, and max results should be numeric.")
})
...
test_that("bad API call returns error", {
  expect_output(suppressMessages(get_orders_account(list(),
                                                    encrypted_account_id = "test")),
                regexp = "Unauthorized")
})
