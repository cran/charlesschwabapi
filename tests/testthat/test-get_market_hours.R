...
test_that("invalid class type for tokens throws error", {
  expect_error(get_market_hours(1,
                                markets = "test"),
               "Tokens parameter must be a list, markets should be a character vector or string, and date should be a date.")
})
...
test_that("invalid value for market throws error", {
  expect_error(get_market_hours(list(),
                                markets = "error"),
               "Markets must be 'equity', 'option', 'bond', 'future', and/or 'forex'.")
})
...
test_that("invalid value for date throws error", {
  expect_error(get_market_hours(list(),
               ...
               "Date must be today or no greater than one year in the future when not NULL.")
})
# Test 6: Bad API authentication call throws error
test_that("bad API call returns error", {
  expect_output(suppressMessages(get_market_hours(list(),
                                                  markets = "equity")),
